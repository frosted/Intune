function Get-IntunePagedResults {
    ...
    param (
        ...
        [string]$appendURL,
        [Parameter()]
        [string]$label = $appendURL.Split('/')[-1].Split('&')[0].Replace('?', '')
    )
    ...
    $commandTime = Measure-Command {
        do {
            # Show loading bar
            Write-Host -NoNewline ("`rLoading $($label) page {0} {1}                                                           "`
                    -f ($pageCount + 1), $loadingChars[$loadingIndex])
            ...
            if ($pageCount -eq 21) {
                ...
                do {
                    ...
                    $loadingIndex = ($loadingIndex + 1) % $loadingChars.Count
                    start-sleep -Milliseconds 200 # Optional: to avoid hitting the API too fast
                } while ((get-date) -lt $pauseTimeStamp.AddSeconds(60)) # Pause for 60 seconds after page 30
                
            }
        } while ($url)
    }

    Write-Host "`rLoading $($label) complete. Fetched $pageCount page(s). Time elapsed: $($commandTime.toString()).        "
